...
void SET_D2_WHITE(void);                                        // Function Prototyping Area End
...
{   
    ...
    char ON = D_POINT= 0; 
    char OFF = D_POINT = 1; 
  //  float RREF = 100;                                            // Resistance used to measure unknown resistor in kOhms
  //  float VREF = 4.096;
    
        while(1)
        {
            Select_ADC_Channel(4);                              // Select AN1 for ADC conversion
            int STEP = get_full_ADC(); 
            float VOLT = (round((STEP * 4.0)))/1000;
            float RL = (100 * VOLT) / (4.096 - VOLT);           // Calculation of Resistance using Voltage Divider Rule
