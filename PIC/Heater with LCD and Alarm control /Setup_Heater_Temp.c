...
void Increase_Heater_Temp()
{
        ...
        if (setup_heater_set_temp > 120) setup_heater_set_temp = 120;       // check if it is greater than 120. If so, limit to 120
...
void Decrease_Heater_Temp()          
{
        ...
        if (setup_heater_set_temp < 50) setup_heater_set_temp = 50;         // check if it is less than 50. If so, limit to 50
    	Update_Setup_Heater_Screen();                                       // Update screen with latest info
...
{ 
    ...
    strcpy(txt, "ECE3301L FALL'22 S3\0");                    // Text displayed 
